...
freeVars_e lv (EVar v)
  | Set.member v lv = (Set.singleton v, AVar v)
...
freeVars_e lv (ELet recursive defns body) =
  ...
  where
    ...
    defnsFree
      | recursive = freeInValues Set.\\ binderSet
...
freeVars_e lv (ECase e alts) = (freeVarsOf e' `Set.union` free, ACase e' alts')
  where
    e' = freeVars_e lv e
    alts' =
      [ (tag, args, freeVars_e (lv `Set.union` Set.fromList args) e)
      | (tag, args, e) <- alts
      ]
...
abstract_e (_, ALet recursive defns body) =
  ELet
    recursive
    [(name, abstract_e body) | (name, body) <- defns]
    (abstract_e body)
